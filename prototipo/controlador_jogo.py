...
class Player(pygame.sprite.Sprite):  # Desenha um player
    ...
    def move(self):
        # a aceleração continua em 0.5 é o que define a gravidade para o player no jogo
        self.acc = vec(0, 0.5)
        ...
        if self.pos.x + (self.size[0]/2) > WIDTH:
            # Impede o crossmap para direita
            self.pos.x = WIDTH - (self.size[0]/2)
        if self.pos.x - (self.size[0]/2) < 0:
            # impede o crossmap para a esquerda
            self.pos.x = 0 + (self.size[0]/2)
...
# Define as plataformas que compõem essa fase
level_plataforms = floor, plat1, plat2, plat3
